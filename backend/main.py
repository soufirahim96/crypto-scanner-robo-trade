...
def get_robo_schedules():
    """VERSION 116: GET ROBO TRADE SCHEDULES WITH DYNAMIC AUTO-RESETTING 2-MINUTE TIMER"""
    now_ts = time.time()
...
def run_robo_trade_loop():
    """
    VERSION 116 HYBRID LOGIC — GOD OF TRADE MASTER ENGINE (LIVE RAILWAY DEPLOYMENT V116)
    ...
    time.sleep(10)
    print("[V116 HYBRID LOGIC] Starting Version 116 Hybrid Autonomous Robo Trade Engine...")

    # Startup — clear old pending schedules so fresh plans are generated immediately
    try:
        db_manager.clear_robo_schedules()
        print("[V116 STARTUP] Robo schedules refreshed. Active holdings & history preserved.")
    except Exception as ex_fresh:
        print(f"[V116 STARTUP ERROR] {ex_fresh}")
    ...
    while True:
        try:
            ...
            for participant in participants:
                ...
                # V116 HIGH-FREQUENCY ENTRY SCANNING ENGINE
                if needs_reanalysis and not is_circuit_broken:
                    ...
                    for c in valid_coins:
                        ...
                        # STAGE 10 RULE #2 & STAGE 2/4: BEARISH & DEEP BEARISH HARD VETO SHIELD (V116)
                        ...
                        if reg:
                            ...
                            if reg_status in ("BEARISH", "DEEP_BEARISH"):
                                ...
                                if time_since_exit >= 180 and is_15m_green and has_volume_consistency and not is_bearish:
                                    reg["status"] = "CLEARED"
                                    print(f"[V116 REGISTRY RECOVERY] {sym_c} recovered from {reg_status} -> CLEARED (15M Green + Volume Consistency confirmed)")
                                else:
                            ...
                            if reg_status in ("PULLBACK_WATCH", "EARLY_WARNING_PULLBACK"):
                                ...
                                if time_since_exit < 180:
                                    ...
                                    print(f"[V116 REGISTRY CLEARED] {sym_c} cleared for re-entry! Reason: {'Deep drop <= -5.0%' if has_dropped_deep_5_0 else '15M Green + Volume Consistency confirmed'}")
                                else:
                        ...
                        # V116 Order Flow Scoring Engine Across All 481+ Coins (Max 10.0 Pts)
                        sweep_pts   = 2.5 if chg > 1.2 else 2.2
                        cvd_pts     = 2.5 if vol > 7500000 else 2.1
                        funding_pts = 2.0 if chg >= 0 else 1.8
                        bos_pts     = 1.5 if chg > 1.8 else 1.2
                    ...
                    for score_val, c in scored_coins:
                        ...
                        # STAGE 10 RULE #6: ADAPTIVE ENTRY PRICE GUIDANCE (V116: 0.9850 Pullback, 0.9935 Bull/Side, 0.9990 Grade S)
                        ...
                        if "GOD" in participant:
                            tier_str = f"👑 GRADE S ({score_val:.1f} Pts) V116 HYBRID" if score_val >= 9.5 else f"GRADE A ({score_val:.1f} Pts)"
                        else:
                    ...
                    if new_sched:
                        ...
                        print(f"[V116 HYBRID SCHEDULE] {participant} [{market_regime}] refreshed {len(new_sched)}-coin plan. Top: {new_sched[0]['confluence_score']} Pts")
                        ...
                        if not is_already_held:
                            ...
                            if open_count >= 4 and p_holdings:
                                ...
                                for r_idx in range(min(num_to_rotate, len(holding_pnls))):
                                    ...
                                    print(f"[V116 GRADE S ROTATION] Exited {worst_h['symbol']} PnL:{worst_pct:+.2f}% to free slot for 2-Lot Grade S {s_sym}")
                    ...
                    for sched in pending:
                        ...
                        if t_now and t_now.get("price", 0) > 0:
                            ...
                            if curr_price > 0 and curr_price <= entry:
                                ...
                                open_count += num_lots
                                print(f"[V116 ENTRY] {participant} [{market_regime}] bought {sym} @ ${curr_price:.5f} ({num_lots}-Lot ${capital:.2f} Spot)")
                                if open_count >= 5:
                                    break
                ...
                for re_sym, re_data in smart_reentry_pending.get(participant, {}).items():
                    ...
                    if is_gr_s_re:
                        ...
                        if b1 and b2 and open_count < 5 and not is_circuit_broken:
                            ...
                            print(f"[V116 GRADE S RE-ENTRY] {participant} continuous re-entry {re_sym} @ ${re_price:.5f} (-0.5% dip)")
                            continue
                    ...
                    if should_exit:
                        ...
                        if net_pnl < 0:
                            ...
                            if loss_cnt >= 2:
                                ...
                                print(f"🛑 [V116 2X LOSS BLACKLIST] {sym} suffered {loss_cnt} consecutive losses! Blacklisted from all schedules & entries for 30 minutes.")
                        else:
                        ...
                        if net_pnl < 0 or exit_tag in ("PULLBACK_WATCH", "EARLY_WARNING_PULLBACK", "BEARISH"):
                            ...
                            print(f"📌 [V116 EXIT REGISTRY TAGGED] {sym} tagged as {reg_status} (180s Hard Cooldown Active)")
